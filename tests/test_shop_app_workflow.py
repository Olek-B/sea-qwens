...
@pytest.fixture
def shop_spec():
    """Fixture: Create a shop app spec"""
    shop_spec = {
        "name": "Basic Shop App",
...
def test_shop_app_spec(shop_spec):
...
@pytest.fixture
def tasks(shop_spec):
    """Fixture: Decompose shop spec into tasks"""
    atomizer = Atomizer()
    with patch.object(
        atomizer,
        "_mock_decompose",
        ...
        return atomizer.decompose(shop_spec)


def test_atomizer_decompose(tasks):
...
def test_kanban_dispatch(tasks):
    ...
    with (
        patch.object(dispatcher.tool_manager, "select_tool") as mock_select,
...
def test_validator():
    ...
    valid_contract = {"files": [{"path": "main.py", "must_exist": True}]}
...
def test_full_workflow(shop_spec, tasks):
    ...
    test_shop_app_spec(shop_spec)

    # Step 2: Decompose into tasks
    test_atomizer_decompose(tasks)
