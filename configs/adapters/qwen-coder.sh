#!/usr/bin/env bash
# Adapter for Qwen Coder CLI
# Usage: bash qwen-coder.sh --prompt "..." --cwd /path [--tool_id id]
set -euo pipefail
...
# Qwen Coder-specific setup: PATH and config directories
...
exec qwen -m qwen-coder --non-interactive --prompt "$PROMPT" --cwd "$CWD"
